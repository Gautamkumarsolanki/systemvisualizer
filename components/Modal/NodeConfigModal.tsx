...
const NodeConfigModal = ({
  ...
  const handleChange = (side: string, value: ConnectorType) => {
    if (!nodeMetaData) return;
    setNodeMetaData({ ...nodeMetaData, [side]: value });
  };

  const handleNameChange = (value: string) => {
    if (!nodeMetaData) return;
    setNodeMetaData({ ...nodeMetaData, name: value });
  };
      ...
      <div className="relative z-10 w-full max-w-md rounded-2xl bg-white shadow-2xl">
        ...
        <div className="flex items-center justify-between border-b px-5 py-4">
          <h2 className="text-lg font-semibold text-gray-800">
            {nodeMetaData?.title ?? "Node Configuration"}
          </h2>
              ...
              {sides.map((side) => (
                ...
                >
                  ...
                  <div className="flex gap-1">
                    {options.map((option) => {
                      const active = nodeMetaData?.[side] === option;

                      return (
                        <button
                          key={option}
                          onClick={() => handleChange(side, option)}
                          className={`
